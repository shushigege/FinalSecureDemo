...
import com.example.secure_demo.util.tool.ConvertBytesToBase64;
...
//	}
    ...
    //解密函数
    public static String decodeRSA(RSAKeyData key_data, String src) {
        if (key_data == null) {
            //默认的密钥对
            key_data = new RSAKeyData();
            key_data.public_key = "10001";
        ...
        try {
            PrivateKey key = getPrivateKey(key_data.modulus, key_data.private_key, key_data.radix);
            String rev = decodeURL(new StringBuilder(src).reverse().toString());
            byte[] en_byte = decrypt(key, rev.getBytes());
            String base64 = decodeURL(ConvertBytesToBase64.BytesToBase64String(en_byte));
            return base64;
        } catch (Exception e) {
            e.printStackTrace();
            return "RSA解密失败";
    ...
    private static String encodeURL(String str) {
        ...
        try {
            encode_str = URLEncoder.encode(str, "GBK");
        } catch (Exception e) {
            e.printStackTrace();
        }
        return encode_str;
    ...
    private static String decodeURL(String str) {
        ...
        try {
            decode_str = URLDecoder.decode(str, "GBK");
        } catch (Exception e) {
            e.printStackTrace();
        }
        return decode_str;
